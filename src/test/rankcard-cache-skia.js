...
class ReactCache {
	...
	async create(background) {
		const canvas = new Canvas(config.widthShadow, config.heightShadow);
		...
		ctx.fill();
		ctx.translate(-config.shadow, -config.shadow);
		this.cache[background] = ctx.getImageData(0, 0, config.widthShadow, config.heightShadow);
	}
}
...
async function main() {
	...
	var width = config.widthShadow;
	var height = config.heightShadow;
	...
	ctx.translate(config.shadow, config.shadow);
	//let avatar = await loadAvatar(user_id, user_avatar, user_discriminator, 256);
	...
	//sdrawCircleImage(ctx, avatar, avatarX, avatarY, avatarRadius); // avatar
	...
	// console.time("convert + save to png");
	await canvas.toBuffer("png");
	// console.timeEnd("convert + save to png");
